...
function CountriesPage() {
  ...
  if (error) {
    return (
      <div className="p-4 rounded-lg border border-red-700 bg-red-950/40 dark:text-red-200">
        <h1 className="text-lg font-semibold">Failed to load countries</h1>
        <p className="text-sm mt-1">{error.message}</p>
        <button
          type="button"
          onClick={() => refetch()}
          className="mt-4 px-4 py-2 rounded bg-red-500 text-slate-950 font-semibold cursor-pointer"
        >
          Retry
        </button>
      </div>
